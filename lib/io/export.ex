defmodule Plsm.IO.Export do
  ...
  def prepare(table, project_name) do
    ...
    output = output <> belongs_to_output <> new_line
  ...
  defp use_ecto_schema do
    two_space("use Ecto.Schema\n")
  end

  defp import_ecto_changeset do
  ...
  defp changeset_list(columns) do
    changeset_items = Enum.reduce(columns, "", fn x, acc -> acc <> ":#{x.name}, " end)
    "[" <> String.slice(changeset_items, 0, String.length(changeset_items) - 2) <> "]"
  end
